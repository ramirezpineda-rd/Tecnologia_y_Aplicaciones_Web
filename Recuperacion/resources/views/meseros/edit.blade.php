...
@section('content')
    ...
    <div class="col-md-8 mx-auto bg-danger text-white p-3">
        <h2 class="text-left mb-7">Editar mesero</h2>
    </div>
    <!-- Diseñar el formulario para guardar receta -->
    <div class="row justify-content-center mt-5">
        <div class="col-md-8">
        <form method="POST" action="{{route('meseros.update', compact('mesero'))}}"><!--Validacion -->
        @method('PUT')
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror"
                placeholder="Nombre del mesero" value={{$mesero->nombre}}>
        ...
        <div class="form-group">
            ...
            <input type="text" name="edad" class="form-control @error('edad') is-invalid @enderror"
                placeholder="Edad del mesero" value={{$mesero->edad}}>
                <!--Directiva de Laravel para poner un mensaje de error-->
                @error('edad')
                    <span class="invalid-feedback d-block" role="alert">
                    <!--Ponemos un mensaje generado por Laravel-->
        ...
        <div class="form-group">
            ...
            <input type="text" name="telefono" class="form-control @error('telefono') is-invalid @enderror"
                placeholder="Telefono del mesero" value={{$mesero->telefono}}>
                <!--Directiva de Laravel para poner un mensaje de error-->
                @error('telefono')
                    <span class="invalid-feedback d-block" role="alert">
                    <!--Ponemos un mensaje generado por Laravel-->
        ...
        <div class="form-group">
            ...
            <input type="text" name="correo" class="form-control @error('correo') is-invalid @enderror"
                placeholder="Correo del mesero" value={{$mesero->correo}}>
                <!--Directiva de Laravel para poner un mensaje de error-->
                @error('correo')
                    <span class="invalid-feedback d-block" role="alert">
                    <!--Ponemos un mensaje generado por Laravel-->
        ...
        <div class="form-group">
            ...
            <select name="mesas"
                ...
                @foreach($mesas as $id => $mesa)
                    <option value="{{$id}}" {{ $mesero->mesa_asignada == $id ? 'selected' : ''}}>
                        {{$mesa}}
                    </option>
                @endforeach
                </select>
